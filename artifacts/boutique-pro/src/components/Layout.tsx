...
import { Moon, Sun, LayoutDashboard, Package, ShoppingCart, Settings, Menu } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] flex bg-background">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 flex-col fixed inset-y-0 left-0 border-r border-sidebar-border bg-sidebar z-10">
        <div className="p-6 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-primary-foreground font-bold">
            BP
          </div>
          <span className="text-xl font-bold text-sidebar-foreground tracking-tight">Boutique Pro</span>
        </div>
      ...
      <main className="flex-1 md:pl-64 flex flex-col min-h-[100dvh]">
        ...
        <header className="md:hidden flex items-center justify-between p-4 border-b bg-card sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-foreground">Boutique Pro</span>
          </div>
